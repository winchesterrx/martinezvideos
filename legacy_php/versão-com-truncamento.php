...
function truncar_titulo($titulo, $video_id) {
    $titulo_seguro = htmlspecialchars($titulo, ENT_QUOTES, 'UTF-8');
    $limite = 40;
    if (mb_strlen($titulo_seguro) > $limite) {
        $titulo_truncado = mb_strimwidth($titulo_seguro, 0, $limite, "");
        // Ao exceder, adicionamos "..." clicável que leva ao detalhes
        // A cor azul, sem sublinhado, e mesma fonte/tamanho do título será herdada do título
        return $titulo_truncado . '<a href="video_detalhes.php?id=' . $video_id . '" style="color: inherit; text-decoration:none;">...</a>';
    } else {
        return $titulo_seguro;
    }
}

function truncar_descricao($descricao, $video_id) {
    $descricao_segura = htmlspecialchars($descricao, ENT_QUOTES, 'UTF-8');
    $limite = 60;
    if (mb_strlen($descricao_segura) > $limite) {
        $descricao_truncada = mb_strimwidth($descricao_segura, 0, $limite, "");
        // Ao exceder, adicionamos "..." + "ver mais"
        // Cor azul, sem sublinhado, mesma fonte e tamanho da descrição (herdado do <p>)
        return $descricao_truncada . '... <a href="video_detalhes.php?id=' . $video_id . '" style="color:blue; text-decoration:none;">ver mais</a>';
    } else {
        return $descricao_segura;
    }
}

function truncar_com_ver_mais($texto, $limite, $video_id) {
    // Se o texto for maior que o limite, precisamos truncar
    if (mb_strlen($texto) > $limite) {
        // mb_strimwidth corta a string no limite de caracteres sem quebrar multibyte
        $texto_truncado = mb_strimwidth($texto, 0, $limite, "");
        // Adicionamos reticências e o link "Ver mais"
        $texto_truncado .= "... <a href=\"video_detalhes.php?id=$video_id\" style=\"text-decoration:none; color: #007bff;\">Ver mais</a>";
        // Retornamos o texto truncado, aplicando htmlspecialchars para segurança
        return htmlspecialchars($texto_truncado, ENT_QUOTES, 'UTF-8');
    } else {
        // Se não precisar truncar, retorna o texto original protegido com htmlspecialchars
...
.video-item {
    ...
    background: #fff;
    border-radius: 10px;
    padding: 10px; /* Diminuir padding de 15px para 10px */
    text-align: center;
    box-shadow: 0px 4px 10px rgba(0, 0, 0, 0.1);
    width: 100%;
    height: 520px; /* Ajuste aqui conforme necessário */
    overflow: hidden;
}
...
.video-title {
    ...
    font-weight: 500;
    color: #333;
    margin: 0.3rem 0;  /* Diminuir de 0.5rem para 0.3rem */
    line-height: 1.2;
}

/* Opcionalmente, diminuir um pouco a fonte da descrição e margens */
.video-item p {
    font-size: 0.9rem; /* Levemente menor */
    margin-bottom: 0.3rem; /* Menos espaço entre parágrafos */
    ...
    <div class="row g-3 mt-4">
        <?php if (mysqli_num_rows($videos_result) > 0): ?>
            <?php while ($video = mysqli_fetch_assoc($videos_result)):
                $titulo_curto = truncar_titulo($video['titulo'], $video['id']);
                $descricao_curta = truncar_descricao($video['descricao'], $video['id']);
            ?>
                <div class="col-md-4">
                    <div class="video-item" onclick="window.location.href='video_detalhes.php?id=<?= $video['id'] ?>'">
                        <video controls>
                            <source src="<?= htmlspecialchars($video['url_video']) ?>" type="video/mp4">
                        </video>
                        <h3 class="video-title"><?= $titulo_curto ?></h3>
                        <p><?= $descricao_curta ?></p>
                        ...
                        <p class="click-instruction" style="font-size: 0.75rem; color: rgba(0,0,0,0.3); margin-top: 0.5rem; font-style: italic; line-height: 1.2;">
                            Clique no vídeo ou no contêiner para ver mais detalhes
                        </p>
                    </div>
                </div>
            <?php endwhile; ?>
