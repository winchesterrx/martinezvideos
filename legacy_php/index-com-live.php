    ...
    <div>
        <?php if ($is_logged_in): ?>
            ...
            <?php if ($usuario_adm): ?>
                ...
                <button class="btn" onclick="openLiveModal()">
        <i class="fas fa-broadcast-tower"></i> Transmissão ao Vivo
    </button>
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuButton">
                        <li><a class="dropdown-item" href="listar_usuarios.php">Cadastro de Usuários</a></li>
...
<div id="liveModal" class="modal">
    <div class="modal-content">
        <div class="modal-header">
            <h2 class="modal-title"><i class="fas fa-broadcast-tower"></i> Configurar Transmissão</h2>
            <button class="btn-close" onclick="closeLiveModal()">&times;</button>
        </div>
        <div class="modal-body">
            <label for="liveURL" class="form-label"><i class="fab fa-youtube"></i> URL da Transmissão:</label>
            <input type="text" id="liveURL" class="form-control" placeholder="Cole o link da sua live no YouTube" required>
            <div class="modal-actions">
                <button class="btn btn-primary" onclick="startLive()">
                    <i class="fas fa-play-circle"></i> Iniciar Live
                </button>
                <button class="btn btn-danger" onclick="stopLive()">
                    <i class="fas fa-stop-circle"></i> Encerrar Live
                </button>
            </div>
            <div class="modal-actions">
                <button class="btn btn-secondary" onclick="muteLive()">
                    <i class="fas fa-volume-mute"></i> Mutar
                </button>
                <button class="btn btn-secondary" onclick="unmuteLive()">
...
$live_query = "SELECT url, titulo, descricao FROM transmissao_ao_vivo WHERE ativo = 1 ORDER BY created_at DESC LIMIT 1";
    ...
    <section class="live-wrapper">
        <div class="live-container">
            <div class="live-header">
                <h3 class="live-title">
                    <span class="live-indicator"></span> <?= htmlspecialchars($live_titulo) ?>
                </h3>
                <div class="live-controls">
                    <button onclick="openLivePopup()" class="btn-live"><i class="fas fa-external-link-alt"></i></button>
                    <button onclick="toggleMute()" class="btn-live"><i id="muteIcon" class="fas fa-volume-up"></i></button>
                    <button onclick="goFullscreen()" class="btn-live"><i class="fas fa-expand"></i></button>
                </div>
            </div>

            <div class="live-player">
                <iframe id="livePlayer" width="100%" height="220"
                    src="<?= htmlspecialchars($live_url) ?>?enablejsapi=1" 
...
<script>
    function openLivePopup() {
        ...
        window.open(liveURL, '_blank', 'width=500,height=300');
    }
    ...
    function goFullscreen() {
        document.getElementById("livePlayer").requestFullscreen();
    }
</script>


<!-- final do conteiner e funções da live -->
...
<div class="footer" id="footer">
    <p>&copy; 2024 Gabriel Bárbara. Todos os direitos reservados.</p>
    <p>
...
window.onscroll = () => {
    ...
    function startLive() {
        ...
        // Salvar a URL da live na sessão usando AJAX
        fetch('set_live.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: 'live_url=' + encodeURIComponent(liveURL)
        })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                window.location.reload(); // Atualiza a página para exibir a live
            } else {
    ...
    function stopLive() {
        ...
        .then(data => {
            if (data.success) {
                window.location.reload(); // Atualiza a página para remover a live
            } else {
